...
async function loadOrders() {
  try {
    ...
    root.innerHTML = `<table><thead><tr><th>Mã</th><th>Khách</th><th>Sản phẩm</th><th>Tổng</th><th>Thanh toán</th><th>Trạng thái</th></tr></thead><tbody>${orders.map((order) => `<tr><td>${order.id.slice(0,8)}</td><td>${safe(order.customer?.fullname)}</td><td>${order.items.map((item) => `${safe(item.productName)} ×${item.quantity}`).join('<br>')}</td><td>${formatMoney(order.totalAmount)}</td><td>${order.paymentStatus}</td><td><select data-order-status="${order.id}">${statusOptions.map((status) => `<option ${status === order.status ? 'selected' : ''}>${status}</option>`).join('')}</select></td></tr>`).join('')}</tbody></table>`;
